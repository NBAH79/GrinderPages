...
namespace Server
{
    ...
    {
        public delegate Task ParserData(Session session, Transfer transfer);
        public delegate Task ParserPong(Session session, byte[] data, uint id);
        ...
        public Task ParseData(Session session, Transfer transfer) => OnData.Invoke(session, transfer);
        public Task ParsePong(Session session, byte[] data, uint id) => OnPong.Invoke(session, data, id);
        public Task ParseLson(Session session, string text) => OnLson.Invoke(session, text);
        public Task ParseUpdate(Session session) => OnUpdate.Invoke(session);
    }
    ...
    public class Listener
    {
        ...
        private HttpListener Initialize(IManager manager, string webaddress, string metafile)
        {
...
<html lang='en'>
    ...
    <body></body>
        ...
        public async Task<int> Node(Session session, ServiceManager parser, CancellationToken token)
        {
