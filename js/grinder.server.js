...
var bisy=false; //если страница переключается а другой поток добавляет элементы
//тут бы токен, да ничего плохого не происходит.
...
class Server {
    ...
    constructor(YYY, KEY) {
        ...
        Server.websocket.onopen = (e) => {
            ...
            var shortlocation=location.pathname.split("/").pop();
            this.doSendString(`${KEY}|${shortlocation}`) //иначе F5 не будет работать, полный путь не принимается с версии 2.0
            //this.doSendString(`${KEY}|${location}`); //до версии 2.0 надо дать знать серверу кто спрашивает и что
        };
                ...
                switch (array.length) {
                    ...
                    case 2:
                        ...
                        if (_command == 'URL') {
                            ...
                            bisy=true; //иначе новая страница может появиться со старой если в этот момент будет апдейт
    ...
    Parse(array) {
        ...
        var counter=0;
        while (bisy && counter<10) { setTimeout(function(){writeToScreen(`BISY:(${counter})`);counter++;}, 60); }; //60ms
        if (counter==10) return;
        //бывает при переключениии страницы
        //а может и еще какая то проблема с длительной задержкой
